...
3) Serves a tiny HTTP API for the 3D page:
   ...
   - GET / or /room_seminarroom.html -> 3D HTML
   - GET /room_seminarroom.xml -> room definition XML
...
# script_dir = .../smart_home_gs/scripts ; smart_home_gs holds `rag`; lapras_root holds `lapras_middleware`.
script_dir = os.path.dirname(os.path.abspath(__file__))
smart_home_gs = os.path.abspath(os.path.join(script_dir, ".."))
lapras_root = os.path.abspath(os.path.join(script_dir, "..", ".."))
for _p in (smart_home_gs, lapras_root):
    if _p not in sys.path:
        sys.path.insert(0, _p)

from lapras_middleware.event import MQTTMessage
try:
    from rag.llm_client import init_llm_client
...
class LiveCaptionEngine:
    ...
    def _ensure_model(self):
        ...
        if self._model_init_failed:
            return None
        if init_llm_client is None:
            self._model_init_failed = True
            logger.error("Caption LLM client import failed; cannot initialize LLM.")
            return None
        try:
            logger.info("Initializing caption LLM client model=%s", self.model_name or "<env default>")
            self._llm = init_llm_client(model=self.model_name, base_url=self.base_url)
            return self._llm
...
def main():
    ...
    parser.add_argument("--caption-model", default=None, help="Served caption model name (overrides LLM_MODEL env)")
    parser.add_argument("--caption-base-url", default=None, help="vLLM OpenAI base URL (overrides LLM_BASE_URL env)")
    parser.add_argument(
        "--caption-gpu-id",
        default="",
    ...
    html_path = (
        ...
        else os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "web", args.visualization_file)
    )
    ...
    print(f"XML URL:     http://{args.http_host}:{args.http_port}/room_seminarroom.xml")
