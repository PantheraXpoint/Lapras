...
# Ensure the smart_home_gs dir (parent of scripts/) is importable so the local
# `rag` package resolves when run as `python scripts/sensor_caption_infer.py`.
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
SMART_HOME_GS = os.path.dirname(SCRIPT_DIR)
if SMART_HOME_GS not in sys.path:
    sys.path.insert(0, SMART_HOME_GS)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--model", type=str, default=DEFAULT_MODEL, help="Served model name (overrides LLM_MODEL env).")
    parser.add_argument("--base-url", type=str, default=None, help="vLLM OpenAI base URL (overrides LLM_BASE_URL env).")
